"""Rico Bot Fractal Swing Strategy — ported from Node/TypeScript.
2L/2R fractal swings on daily ES bars.
Sweep detection: wick beyond swing level, close back inside, min 8-tick range.
BOS: break + close beyond swing, body >= 6 ticks, close in extreme 25% of bar.
CHoCH: structural reversal (lower lows -> break high = bull, higher highs -> break low = bear).
Bias: close vs midpoint of last swing range.
HTF gating: 5L/5R fractals (proxy for 4H) must not be bearish, 3L/3R (proxy for 1H) must confirm.
Regime filter: SMA20 range filter as VWAP proxy.
SL: swing invalidation level. TP: 1.5x risk. Max SL: 16 ticks (8 points on ES).
Costs: $5 RT commission + $12.50 slippage per side per contract. ES point value = $50.
Starting capital $100K. Topstep sim: $150K, $4,500 trailing DD, $9,000 target.
"""
import sys
import os
...
STRATEGY_NAME = "Rico Fractal Swings (ES)"
...
def run_strategy(es):
    ...
    fh_2, fl_2 = find_fractals(highs, lows, 2, 2)
    fh_3, fl_3 = find_fractals(highs, lows, 3, 3)
    fh_5, fl_5 = find_fractals(highs, lows, 5, 5)
    trades = []
    ...
    trade_direction = 0
    cooldown_until = 0
    for i in range(REGIME_SMA_PERIOD, n):
        active_sh_2 = [(idx, p) for idx, p in fh_2 if idx <= i - 2]
        active_sl_2 = [(idx, p) for idx, p in fl_2 if idx <= i - 2]
        active_sh_3 = [(idx, p) for idx, p in fh_3 if idx <= i - 3]
        active_sl_3 = [(idx, p) for idx, p in fl_3 if idx <= i - 3]
        active_sh_5 = [(idx, p) for idx, p in fh_5 if idx <= i - 5]
        active_sl_5 = [(idx, p) for idx, p in fl_5 if idx <= i - 5]
        ...
        if sma_dist > 2.0 * avg_range:
            continue
        htf_bias = calc_bias(closes[i], active_sh_5[-10:], active_sl_5[-10:])
        mtf_bias = calc_bias(closes[i], active_sh_3[-10:], active_sl_3[-10:])
...
def run():
    ...
    print(f"\nStrategy parameters:")
    print(f"  Fractal: 2L/2R (primary), 3L/3R (MTF gate), 5L/5R (HTF gate)")
    print(f"  Sweep min range: {SWEEP_MIN_RANGE_TICKS} ticks")
    ...
    print(f"  Costs: ${COST_PER_TRADE:.0f}/trade (${COMMISSION_RT} comm + 2x${SLIPPAGE_PER_SIDE} slip)")
    ...
    if len(trade_list) == 0:
        ...
        print("  This likely means the HTF gating + sweep/BOS filters are too")
        print("  restrictive on daily bars. Try relaxing parameters.")
        print("\n  VERDICT: NO EDGE (insufficient trades)")
        return
    stats = summary(trade_pnls=trade_pnls, equity_curve=equity, position_series=pos)
    ...
    if len(trade_list) > 0:
        ...
        print(f"\n  Year-by-year:")
        print(f"  {'Year':>6} {'Trades':>8} {'Win%':>8} {'Total P&L':>12} {'Avg P&L':>10}")
        print(f"  {'-'*6} {'-'*8} {'-'*8} {'-'*12} {'-'*10}")
        for year, grp in trade_list.groupby("year"):
            yr_trades = len(grp)
            yr_wr = (grp["pnl"] > 0).mean()
            yr_total = grp["pnl"].sum()
            yr_avg = grp["pnl"].mean()
            print(f"  {year:>6} {yr_trades:>8} {yr_wr:>7.1%} ${yr_total:>11,.0f} ${yr_avg:>9,.0f}")
    print(f"{'='*70}")
    ...
    if len(trade_list) > 0:
        ...
        csv_path = os.path.join(RESULTS_DIR, "rico_fractal_trades.csv")
